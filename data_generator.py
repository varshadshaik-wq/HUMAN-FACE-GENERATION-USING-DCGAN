...
class ImageDataGenerator:
    def __init__(self, batch_size, height, width, z_dim, shuffle=True,
        ...
        if mode == 'train':
            self.file_folder = 'E:/takeoff/AgeCGan/checkpoints/tools/train'
            ...
            self.file_folder = 'E:/takeoff/AgeCGan/checkpoints/tools/test'
    ...
    def load_train_imgs(self, data_dir, img_size=128):
        ...
        for i in range(len(paths)):
            ...
            if img is not None:
                ...
                img -= self.mean
                imgs.append(img)
                read_paths.append(os.path.join(data_dir, paths[i]))
